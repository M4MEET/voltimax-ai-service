...
ALWAYS available (with or without verified order):
  "escalation_ticket" — wants to be CONNECTED to a human agent NOW, open a support ticket, says "support kontaktieren", "mit einem Mitarbeiter sprechen", "Ticket erstellen". This is an ACTION (start a ticket), not an information request. Do NOT use for asking HOW to contact support — "Wie erreiche ich den Kundendienst?", "Kontaktdaten", "Telefonnummer", "Hotline", "Öffnungszeiten" are informational → action "none" (AI gives the contact details).
